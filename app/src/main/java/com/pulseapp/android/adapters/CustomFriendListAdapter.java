 ...
 */
public class CustomFriendListAdapter extends HeaderRecyclerViewAdapterV2{
    ...
    public CustomFriendListAdapter(Fragment fragment, Context context, List<ListRoomView> list){
        mContext = context;
        friendList = list;
        viewControlsCallback = (OnListModified)fragment;
    }

    public class CustomFriendListViewHolder extends RecyclerView.ViewHolder{
        ...
        public CustomFriendListViewHolder(View itemView) {
            super(itemView);
    ...
    public RecyclerView.ViewHolder onCreateBasicItemViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == ITEM_VIEW_TYPE_HEADER) {
            ...
            return new CustomFriendListViewHolder(item);
        }
    }
    ...
    public void onBindBasicItemView(final RecyclerView.ViewHolder holder, final int position) {
        ...
        if (itemViewType == ITEM_VIEW_TYPE_HEADER) {
            ...
            if (friendList.get(position).type == 2) {
                ((CustomFriendListViewHolder) holder).itemView.setVisibility(View.GONE);
            } else {
                if (friendList.get(position).isChecked){
                    ((CustomFriendListViewHolder) holder).checkBox.setChecked(true);
                } else {
                    ((CustomFriendListViewHolder) holder).checkBox.setChecked(false);
                }
                ((CustomFriendListViewHolder) holder).profileName.setText(friendList.get(position).name);
                Picasso.with(mContext).load(friendList.get(position).imageUrl).fit().transform(new RoundedTransformation()).into(((CustomFriendListViewHolder) holder).profileImage);
                ((CustomFriendListViewHolder) holder).itemView.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (((CustomFriendListViewHolder) holder).checkBox.isChecked()) {
                            ((CustomFriendListViewHolder) holder).checkBox.setChecked(false);
                        ...
                        viewControlsCallback.onItemClicked(friendList.get(position), ((CustomFriendListViewHolder) holder).checkBox.isChecked());
                    }
                });
            }
        }
